...
int sketcherboard[ALPHABET_SIZE]={0};
// Input and output buffers
char input_buffer[256] = {0};
char output_buffer[256] = {0};
bool sketcher_board_on=false;
...
void initialize_sketcherboard(const char *pairs) {
    // Reset the plugboard to no swaps
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        pairings[i] = 0;
    }
    ...
    for (int i = 0; i < strlen(pairs); i += 3) {
        if (pairs[i+1] == ' ' && i+2 < strlen(pairs)) {
            ...
            if (first >= 0 && first < ALPHABET_SIZE && 
                second >= 0 && second < ALPHABET_SIZE) {
                // Set up the bidirectional swap
                sketcherboard[first] = second - first;
                sketcherboard[second] = first - second;
            }
        }
    }
}
...
int plug_swap(int input) {
    ...
    if(sketcher_board_on) return input+sketcherboard[input];
...
void spin_rotors() {
    ...
    else if(rotor_offsets[1]==turnovers[1]){
        rotor_offsets[2]=(rotor_offsets[2]+1)%ALPHABET_SIZE;   
        rotor_offsets[1]=(rotor_offsets[1]+1)%ALPHABET_SIZE;
    }
...
#define PT_YIELD_TIME_msec(pt, ms) do { pt->yield_state = __LINE__; return 1; case __LINE__: } while(0);
...
int main() {
    ...
    while (true) {
        ...
        if (command[0] == '1') {
            ...
            printf("Write the sketcher board setting.(e.g., 'A B C D' to swap A-B and C-D): ");
            getchar(); // Clear newline
            fgets(sketcher_board, sizeof(sketcher_board), stdin);
            sketcher_board_on=true;
            initialize_sketcherboard(sketcher_board);
            printf("Sketcherboard configuration set\n\n");
        }
        else if (command[0] == '5') {
