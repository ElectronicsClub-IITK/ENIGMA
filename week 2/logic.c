...
// Advance the rotors
void spin_rotors() {
    bool middle_at_turnover = (rotor_offsets[1] == turnovers[1]);
    bool right_at_turnover = (rotor_offsets[0] == turnovers[0]);

    // Advance middle rotor if right rotor is at turnover (always) — this enables double stepping
    if (right_at_turnover || middle_at_turnover) {
        rotor_offsets[1] = (rotor_offsets[1] + 1) % ALPHABET_SIZE;
    }

    // Advance left rotor only if middle rotor was at its turnover position
...
int PT_Encrypt(struct pt *pt, const char *input, char *output) {
    ...
    while (input[encrypt_idx] != '\0') {
        ...
        // If current character is a space, offer plugboard update
        if (input[encrypt_idx] == ' ') {
            printf("\n[SPACE DETECTED] Enter 1 to update plugboard configuration, 0 to continue: ");
            int choice;
            scanf("%d", &choice);
            ...
            output[encrypt_idx] = ' '; // Preserve space
            encrypt_idx++;
            continue;
        }
